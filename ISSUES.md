...
| B50 | `infoServer.py:1310-1350` | 连接主机前缺少确认弹框 | ✅ 已修复：为 SSH/VNC/RDP 连接添加确认弹框 |
...
| F3 | Radmin 连接 | `infoServer.py:820-847` | ❌ 连接类型下拉框有 `Radmin` 选项，但 `connect_server` 无对应分支，点击报错 |
...
### 1. 分组树图标升级
- 将分组树图标从 PNG 图片更换为 Font Awesome 图标字体
- 实现展开/折叠状态下的图标区分显示效果
- 折叠状态：棕色文件夹图标 `fa-folder`
- 展开状态：橙棕色文件夹图标 `fa-folder-open`

### 2. SSH工具路径独立设置
- 在"设置 - 连接工具"中为每种SSH工具提供独立的路径浏览与选择按钮
- 支持工具：XTerminal、PuTTY、MobaXterm、FinalShell、Xshell
- 每种工具可独立配置路径

### 3. 界面颜色设置
- 在"设置 - 界面设置"中增加字体颜色设置（取色方式）
- 增加服务器说明数据框背景色设置
- 增加搜索框背景色设置
...
### 5. 服务器树优化（2026-08-24）
- 修复服务器树右键菜单中"导入服务器"和"导出服务器"菜单项重复的问题
- 调整服务器树类型图标列表头的默认宽度，使其与类型表头的宽度保持一致（50px）
- 在服务器树表头最后添加一列"状态"，为后续检测服务器网络连通性的功能预留
- 调整系统设置窗口的宽度（520→600→1200），确保工具路径和浏览按钮能够完整显示
- 系统设置窗口保存设置后不自动关闭，提升用户体验
- 修复SSH/VNC连接时报 AttributeError: 'too' object has no attribute 'db_path' 的问题
...
| Q4 | 重复代码 | `edit_da` 中 6 个 `elif` 分支结构几乎相同，应抽象为通用方法 |
| Q5 | 重复代码 | `gui_DA.py` 每个方法都重复 `sqlite3.connect` → `cursor` → `execute` → `close` 模式，应提取上下文管理器 |
| Q6 | 异常处理 | 多处 `except Exception as e` 过于宽泛，吞掉所有异常 |
| Q7 | 资源管理 | `gui_DA.py` 未使用 `with` 语句管理数据库连接，异常时连接不关闭 |
| Q8 | 调试代码 | `infoServer.py` 和 `gui_DA.py` 中大量 `print()` 未清理 |
| Q9 | 注释 | `pack_propagate` 注释与代码行为相反（见 B20） |
...
| Q13 | 类型安全 | `server_tree.item(...)['values'][1]` 取值后未做类型转换，`isinstance` 检查后直接 `raise ValueError` 而非友好提示 |
...
| # | 风险 | 说明 | 建议 |
|---|------|------|------|
| S1 | 密码明文存储 | `servers.password` 明文存入 sqlite | 使用 `cryptography` 库加密，或至少使用 base64 + 盐值混淆 |
| S2 | RDP 文件含明文密码 | `tool.py:86` 将密码明文写入 `temp.rdp` | RDP 的 `password 51` 字段应使用加密凭据；临时文件删除时机需确保安全 |
| S3 | SQL 注入 | `gui_DA.py:110` 表名通过 f-string 拼接 | 使用白名单校验表名 |
| S4 | 日志泄露敏感信息 | `infoServer.py` 日志中记录用户名和密码 | 日志中密码应脱敏（如 `***`） |
| S5 | 临时文件竞态 | `tool.py:80` `temp.rdp` 写在固定路径，多实例并发时互相覆盖 | 使用 `tempfile.mkstemp` 生成唯一文件名 |
...
### P0（立即修复，影响基本功能）
1. **F3**：Radmin 连接类型无对应分支，选择 Radmin 连接会报错
2. **S1/S2/S4**：密码明文存储及日志泄露，存在安全风险

### P1（近期修复，影响体验）
3. **F4**：重命名分组功能补全
4. **F5/F6**：导入/导出分组功能补全
5. **F8**：编辑下拉框补充"主机类型"选项
6. **B22**：`setDaemon` 弃用 API 修复
7. **B28**：清理 `print()` 调试输出

### P2（中期优化）
8. **Q4/Q5**：重复代码抽象
9. **Q7**：数据库连接改用 `with` 上下文管理器
10. **C7**：相对路径改绝对路径
11. **F9/F10**：批量添加、导入导出功能补全
12. **F11**：密码加密存储
