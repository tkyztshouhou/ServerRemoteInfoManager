...
| 类型 | 说明 | 状态 |
|------|------|------|
| RDP | Windows 远程桌面，经凭据管理器一键登录（mstsc） | ✅ 已实现 |
...
### RDP 连接原理（2.1 版起）
1. 连接时程序将主机 IP、用户名、密码写入 Windows 凭据管理器（cmdkey）
2. 自动执行 `mstsc /v ip:port` 一键登录，无需手动输入凭据
3. 远程桌面窗口关闭后，程序自动删除刚才写入的凭据
4. 全程不生成含密码的临时文件，密码含特殊字符亦可正常连接
...
**Q：RDP 连接时凭据管理器有什么变化？**
A：连接期间程序会临时写入一条 `TERMSRV/主机IP` 凭据，远程桌面关闭后自动删除。若程序异常退出导致凭据残留，可手动执行 `cmdkey /delete:TERMSRV/主机IP` 清除。
